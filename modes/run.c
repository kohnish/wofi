...
void wofi_run_init(struct mode *this, struct map *config) {
    ...
    wl_list_for_each_safe(node, tmp, cache, link) {
        ...
        struct stat info;
        stat(node->line, &info);
        if (((access(node->line, X_OK) == 0 && S_ISREG(info.st_mode)) || strncmp(node->line, arg_str, strlen(arg_str)) == 0) && !map_contains(cached, full_path)) {
...
size_t wofi_run_get_arg_count(void) { return sizeof(arg_names) / sizeof(char *); }
